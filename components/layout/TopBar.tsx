...
export default function TopBar() {
  ...
  return (
    <div className="hidden h-9 items-center bg-dark text-sm text-white md:flex">
      <div className="section-shell flex w-full items-center justify-between">
        <p className="truncate">{shortAddress}</p>
        <div className="flex items-center gap-4">
          <Link className="inline-flex items-center gap-1 hover:text-accent" href={`tel:${doctor.phone}`}>
            <Phone size={14} />
            {doctor.phoneDisplay}
          </Link>
          <Link className="inline-flex items-center gap-1 hover:text-accent" href={wa} target="_blank">
